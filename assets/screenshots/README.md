...
The 57 PNGs currently in this folder are **synthetic, illustrative
examples** generated by small Pillow scripts included here — not captures
from any real system. Every SOP in the repo has at least one inline
screenshot now. They exist so the repo is immediately useful and every
SOP has a visual reference for "what healthy output looks like," while
you build up a library of real screenshots from your own environment.

The generator is split across several scripts (`generate_screenshots.py`
plus `generate_batch_a.py` through `generate_batch_e.py`), one per batch
of SOPs they were written for — each is self-contained (it includes its
own copy of the `render()` function) so they can be regenerated
independently:

```bash
python3 generate_screenshots.py
python3 generate_batch_a.py
python3 generate_batch_b.py
python3 generate_batch_c.py
python3 generate_batch_d.py
python3 generate_batch_e.py
```

To add a new screenshot, either add an entry to whichever batch script is
most similar in topic, or start a new script from the same `render()`
pattern — there's nothing special about the split, it's just how the work
