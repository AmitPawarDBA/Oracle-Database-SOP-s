...
role during an unplanned outage of the production primary, using either
Data Guard Broker `FAILOVER` (preferred) or manual SQL failover if the
Broker is unavailable, followed by reinstatement of the old primary as a
new standby via Flashback Database once it is recoverable.
...
switches (see `06-data-guard-dr/switchover/01-planned-switchover.md`) or
...
### 5.1 Option A — Broker-Managed Failover (preferred, if Broker/standby reachable)
...
3. Execute the failover:
   ...
   > explicitly reinstated (Section 5.3). Do not attempt to bring the
...
### 5.2 Option B — Manual SQL Failover (Broker unavailable / standby-only access)

Use only if DGMGRL cannot reach the standby or the Broker configuration
itself is unusable (e.g. Broker metadata corruption). This path requires
more manual judgment and should be a fallback, not the default.

5. On the standby, stop managed recovery and inspect what has been
   applied vs. what is available but not yet applied (gap analysis):
   ```sql
   ALTER DATABASE RECOVER MANAGED STANDBY DATABASE CANCEL;

   SELECT thread#, MAX(sequence#) AS last_seq FROM v$archived_log
   WHERE applied = 'YES' GROUP BY thread#;
   ```
6. If additional archived/unapplied redo exists on the standby's
   `standby_archive_dest` or was shipped but not applied, recover through
   the highest available sequence first:
   ```sql
   RECOVER MANAGED STANDBY DATABASE FINISH;
   ```
   `FINISH` applies all remaining available redo (including any redo
   present in the standby redo log buffer via real-time apply) before
   converting the role — this minimizes data loss versus a blind
   activate.
7. Activate the standby as the new primary:
   ```sql
   ALTER DATABASE ACTIVATE STANDBY DATABASE;
   ```
   > **Point of no return:** `ACTIVATE STANDBY DATABASE` performs the
   > equivalent of an incomplete recovery finalization and opens with a
   > new incarnation. Any redo that had not reached or been applied to
   > this standby at the moment of primary loss is **permanently lost**
   > for this incarnation.
8. Open the database and confirm:
   ```sql
   ALTER DATABASE OPEN;
   SELECT database_role, open_mode FROM v$database;
   ```

### 5.3 Post-Failover: Reinstate the Old Primary as New Standby

9. Once the failed primary host/storage is recoverable, determine
   ...
   rebuild.
10. Mount the old primary (do **not** open it):
    ```sql
    SHUTDOWN ABORT;
    STARTUP MOUNT;
    ```
11. Flash back the old primary to the SCN/time just before the failover
    ...
    ```
12. Convert it to a physical standby of the new primary:
    ```sql
    ALTER DATABASE CONVERT TO PHYSICAL STANDBY;
    SHUTDOWN IMMEDIATE;
    STARTUP MOUNT;
    ```
13. Re-add it to the Broker configuration (if it was removed/disabled
    ...
    ```
14. Start managed recovery on the reinstated standby:
    ```sql
    ALTER DATABASE RECOVER MANAGED STANDBY DATABASE USING CURRENT LOGFILE DISCONNECT;
    ```
15. If Flashback Database was **not** enabled, or the flashback window
...
### 5.4 Application, DNS, and Service Cutover

16. Redirect application connections to the new primary's connect
    ...
    old node.
17. Update DNS/load balancer entries or virtual IP if connectivity is
    not handled by TNS role-based routing/FAN.
18. Notify application teams to resume traffic and monitor closely for
...
- **Before Step 3/7 (activation) executes:** if new information arrives
...
- **If reinstatement (Section 5.3) fails partway** (e.g. flashback SCN
...
(Section 5.3) completes, send a final incident update confirming the DR
posture is restored and include a summary of actual data loss for the
post-incident review.

## 9. Known Issues / Gotchas

- `ACTIVATE STANDBY DATABASE` (manual path) does **not** wait for
  additional redo the way `RECOVER ... FINISH` does — always run
  `FINISH` first if there is any chance more redo is available on the
