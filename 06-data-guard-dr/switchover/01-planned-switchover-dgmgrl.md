...
`06-data-guard-dr/failover/01-emergency-failover-dgmgrl.md`) or logical
standby switchover. If the Broker is unavailable or not configured, use
the manual SQL*Plus procedure instead:
`06-data-guard-dr/switchover/02-planned-switchover-manual-sql.md`.
Assumes the standby was built per
